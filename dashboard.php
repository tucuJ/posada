...
$rol = $_SESSION['rol'] ?? '';
$allowedRoles = ['Admin', 'Gerente', 'Recepcion'];
...
try {
    ...
    $query = "SELECT COUNT(*) FROM Reservaciones WHERE Estado IN ('Confirmada', 'Pendiente')";
    $stats['reservas_activas'] = $conn->query($query)->fetchColumn();

    // Habitaciones ocupadas
    $query = "SELECT COUNT(*) FROM Habitaciones WHERE Estado = 'Ocupada'";
    $stats['habitaciones_ocupadas'] = $conn->query($query)->fetchColumn();

    // Ventas de hoy
    $query = "SELECT SUM(Total) FROM Ventas WHERE DATE(FechaHora) = CURDATE() AND Estado = 'Completada'";
    $stats['ventas_hoy'] = $conn->query($query)->fetchColumn() ?? 0;

    // Productos con stock bajo
    $query = "SELECT COUNT(*) FROM Productos WHERE Stock <= StockMinimo AND Activo = 1";
    $stats['stock_bajo'] = $conn->query($query)->fetchColumn();

    // Próximas reservas (5)
    $query = "SELECT r.*, c.Nombre as ClienteNombre, h.Numero as HabitacionNumero
              FROM Reservaciones r
              JOIN Clientes c ON r.ClienteID = c.ClienteID
              JOIN Habitaciones h ON r.HabitacionID = h.HabitacionID
              WHERE r.FechaEntrada >= CURDATE() AND r.Estado = 'Confirmada'
              ORDER BY r.FechaEntrada ASC
              LIMIT 5";
    $proximas_reservas = $conn->query($query)->fetchAll(PDO::FETCH_ASSOC);

    // Últimas ventas (5)
    $query = "SELECT v.*, c.Nombre as ClienteNombre
              FROM Ventas v
              LEFT JOIN Clientes c ON v.ClienteID = c.ClienteID
              WHERE v.Estado = 'Completada'
              ORDER BY v.FechaHora DESC
              LIMIT 5";
    $ultimas_ventas = $conn->query($query)->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container">
    <h2>Dashboard</h2>
    <p class="text-muted">Bienvenido, <?= htmlspecialchars($_SESSION['nombre_completo']) ?></p>
        ...
        <div class="col-md-3">
            <div class="card text-white bg-primary">
                <div class="card-body">
                    ...
                    <p class="card-text display-4"><?= $stats['reservas_activas'] ?></p>
                    <a href="modules/reservas/listar.php" class="text-white">Ver más <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card text-white bg-success">
                <div class="card-body">
                    ...
                    <p class="card-text display-4">$<?= number_format($stats['ventas_hoy'], 2) ?></p>
                    <a href="modules/ventas/listar.php" class="text-white">Ver más <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card text-white bg-warning">
                <div class="card-body">
                    <h5 class="card-title">Hab. Ocupadas</h5>
                    <p class="card-text display-4"><?= $stats['habitaciones_ocupadas'] ?></p>
                    <a href="modules/habitaciones/listar.php" class="text-white">Ver más <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card text-white bg-danger">
                <div class="card-body">
                    ...
                    <p class="card-text display-4"><?= $stats['stock_bajo'] ?></p>
                    <a href="modules/productos/listar.php" class="text-white">Ver más <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><?= htmlspecialchars($reserva['ClienteNombre']) ?></td>
                                        <td><?= $reserva['HabitacionNumero'] ?></td>
                                        <td><?= (new DateTime($reserva['FechaEntrada']))->format('d/m/Y') ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                    ...
                    <?php else: ?>
                        <div class="alert alert-info">No hay reservas próximas</div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>#<?= $venta['VentaID'] ?></td>
                                        <td><?= htmlspecialchars($venta['ClienteNombre'] ?? 'Consumidor Final') ?></td>
...
<?php
// Fecha seleccionada para mostrar ocupación (por defecto hoy)
...
// Mapa rápido de habitación por ID
$habitacionMap = [];
    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <strong><?= htmlspecialchars($habitacion['Numero']) ?></strong><br>
                        <small><?= htmlspecialchars($habitacion['TipoHabitacion']) ?></small>
                    </td>
                    <?php
                    ...
                    for ($dia = 1; $dia <= $dias_en_mes; $dia++) {
                        ...
                        echo "<td class='$celdaClass text-white' title='$tooltip'>$estado</td>";
                    }
                    ?>
                </tr>
                <?php endforeach; ?>
